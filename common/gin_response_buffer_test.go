...
func TestBufferedResponseSpoolStaleCleanupIsPrefixAgeAndLockSafe(t *testing.T) {
	...
	oldDir, err := createPrivateBufferedResponseTempDir(baseDir, bufferedResponseInstancePrefix+"old-")
	require.NoError(t, err)
	...
	activeDir, err := createPrivateBufferedResponseTempDir(baseDir, bufferedResponseInstancePrefix+"active-")
	require.NoError(t, err)
