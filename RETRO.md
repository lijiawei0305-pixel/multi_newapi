...
- **坑点**：① 刻意代价——真废单在网关持续不可查期间会一直占卡单页+重复告警（DedupKey 已防轰炸）：**可见的噪音优于无声的钱损**；凭据修复后一轮即收敛。② **RCG 同族未修（本轮刻意不动）**：`internal/payment/reconcile.go:92-97` 对超龄 created 单**连查单都不做**直接置 failed——形状更糟（AGT/SUB 至少查了、错了才杀；RCG 无条件跳过查单），同样的循环论证成立于「查单在 26h 中持续失败」时。因它在独立包、独立签名（maxAge 是显式参数）、且大概率是审计后续发现的地盘，先报告待并（避免与审计处方撞车）；若后续无对应发现，应按同一不变量修：删 maxAge 短路、超龄单照常查单、错误留 Failed。③ 检法沉淀：凡「终态判定」问一句——**判据是确定性答复还是『没拿到答复』？**后者=把故障当事实。
