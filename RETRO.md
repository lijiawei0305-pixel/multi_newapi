...
### [已解决] DB root 密码在「刚修完 C1」的同一个 compose 文件里仍内联（纵深防御缺口，C1 违例第二例）
- **现象**：修 SESSION_SECRET 内联（上一条）时，**同一个文件**（其注释正论证「内联=把根密钥写进 git」）仍在 4 处内联同一个低熵人工 DB root 密码，均已提交 git：`deploy/docker-compose.test.yml:14`（`SQL_DSN` 内嵌）、`:78`（`MYSQL_ROOT_PASSWORD`）、`deploy/ops/lib.sh:37`（`DB_PASS` 默认值，backup/restore/reconcile/demo 消费）、`deploy/ops/README.md:24`（文档明文）。
- **根因**：修复只盯着报告点名的键（SESSION_SECRET/CRYPTO_SECRET），没把「同文件还有哪些字面量凭据」扫一遍——又是**清单挑选式遗漏**（同 C2/C3/C5/C6 家族）。风险边界（据实）：mysql 服务未发布 `ports:`（仅 compose 网络内可达），单独不可远程利用；但任一容器沦陷（app SSRF/RCE）、日后有人加 `ports:`、或仓库读者+宿主立足点，即可直升唯一生产库 root——与 C1「仓库读者不可信」的既定假设不能同时成立。
- **解决/规避**：四处全清，同款 fail-closed：① compose 两处改 `${MYSQL_ROOT_PASSWORD:?…}`（`SQL_DSN` 内插值 + mysql 服务同源）；② `lib.sh` `DB_PASS` 改三级解析：显式环境变量 > 从 `$ENV_FILE` sed 读 `MYSQL_ROOT_PASSWORD`（容忍引号、取末条） > 缺失即中止；③ ops/README 明文删除。服务器前置已做：`/root/newapi-test/.env`(600) 已写入 `MYSQL_ROOT_PASSWORD`（沿用现值，**未轮换**——存量 MySQL 卷密码烤在数据目录，`MYSQL_ROOT_PASSWORD` 仅新卷首次初始化生效，轮换须库内 `ALTER USER 'root'@'%'/'localhost'` + 同步 `.env` + `up -d app` 三步同做，已在 compose 注释留操作指引，建议低峰另行执行）。验证：本机 `bash -n` + `.env` 解析/缺失中止用例全过；服务器 `/tmp` 副本 `docker compose config` 带 `.env` 两处正确插值、无 `.env` exit=15 拒绝，未触碰在跑栈。
- **升级**：不新增条目——**本例就是 C1 的执行缺口**。C1 的「改 compose/.env.example 前 `git grep` 确认无真值明文」应理解为：修任何一处密钥内联时，**必须顺手 grep 同文件/同目录的全部字面量凭据**（密码、DSN、token），不能只修报告点名的键。相关：[[trial-limit-redis-no-volume-no-release]]（同批 compose 加固）。
...
- **升级**：候选规则——「**任何反滥用/风控维度（去重、限购、指纹、信誉）的信号必须由服务端从可信来源派生，绝不承载客户端自报值**：设备→唯一性键只取**不可由被监管方自选**的信号（如 ClientIP / 网段），**绝不含客户端可自选字段（如 User-Agent）**；身份→authenticated 主体的服务端记录；缺可信来源的维度留空跳过、绝不用请求体字段兜底」。**订正**：初版把 UA 揉进唯一性键是错误的（攻击者每请求换一个 UA 即得一把全新互不碰撞的终身键，绕过成本与自报随机 device_id 相同），已于 audit 2026-07-18 F1 修正（`deviceFingerprint` 改为只哈希 ClientIP、去掉 UA 分量）。与 C4/#365 的「释放侧验归属」互补——本条治「写入侧信号源不可信」。同 C2/C3/C5/C6/C7 的「清单挑选式遗漏」家族（此处漏的是「信号可信性」这一列）。

### [已解决] checkTrialLimit 的「承重假注释」：以「KVCache 无删除原语」论证败者留痕不可避免，而 Del 就是同一批加的——6 行补偿删除被注释「合法化」成取舍
- **现象**：`engine.go` checkTrialLimit 的辩护注释称「KVCache 无删除原语，无法对败者已占的维度做补偿删除」，并据此把「跨账号撞实名/撞设备的败者，其 userK 被永久烧掉」定性为 fail-closed 取舍。但 `Del` 由 714aa6d 加入 `port.go`——比写下该注释的 3dd6b4f 只晚 25 分钟、同一批工作，三实现（接口/MemKV/RedisKV）齐备，port.go 自己还写着「补上此前结构性缺失的释放原语」。HEAD 上两者并存，注释是假的；后续 f5f2f1e 改写措辞删掉了假前提，却保留了不补偿的行为——原语只用于管理员手工兜底端点，从未用于同批论证所称「不可能」的自动补偿。
- **根因**：**承重的推理故障而非运行期故障**——三条 High（败者 userK 永久留痕系列）全部由这条已为假的前提在代码注释里「合法化」，未来读者会据此认为残留不可避免，而非一个 6 行修复。实际伤害面：设备指纹按 ClientIP+UA 派生（粗粒度，见 #377），共享出口 IP 的无辜用户一次碰撞即 user 维被终身烧掉、换干净设备也买不了 Trial、只能走客服释放。
- **解决/规避**：① `checkTrialLimit` 败者/出错路径对**本次已抢占成功**的维度做补偿删除（`claimed` 回滚，2026-07-17）——被删键值都是本人 userID（刚由本次 SetNX 写入，#371 归属改造的直接红利），不会误删赢家或他人占用；补偿尽力而为（Del 失败仅多留痕，fail-closed 方向，可经 ReleaseTrialLimit 解）；同用户并发重复在 userK 首步判负、零键可补。② 重写注释：点名假前提出处（3dd6b4f）与 Del 落地（714aa6d），标注「勿再引用」。③ 新增守卫测试 `TrialLoserCompensatesClaims`（判负路径）+ `TrialSetNXErrorCompensatesClaims`（错误路径，`kvSetNXErrOnKey` 定点注错）；修正 `CrossUserOwnershipGuard` 两处「留痕」旧注释。服务器容器验证：`go vet` + `go test -race ./internal/risk/` + `go test ./internal/...` 全绿；变异 2/2 杀死（分别删两处补偿 Del → 对应测试各自确定性红）。单赢家性质不受影响（共享维度决胜仍全在 SetNX 原子返回值上，Concurrent 两测原样绿）。
- **升级**：候选规则——「**代码注释里任何『X 不可能/无原语/无法做到』式的辩护，在依赖的前提变动（新增原语、新增通道、归属改造）后必须重新审计**：以能力缺失为由接受的每一处残留，都要登记『解锁条件』；能力补上时按登记回收残留，而不是让旧注释继续把缺陷合法化」。与 #403「接旧规格前先核对现状」同族（引用的机制/前提是否还成立），但本条特指**自家注释**成为过时权威。同步 memory `trial-dedup-setnx-discarded-concurrent-bypass`。
...
### [已解决] 同代码三绿两红的计费投影测试：Unix() 整秒截断差把 use_time 断言钉在时间刀锋上（2026-07-26）
- **现象**：依赖修复推送后 Backend job 连红两次,失败点 `TestSynchronousProjectionLogFailureReplaysCompletePayloadExactlyOnce`(service)断言 use_time `expected 3 actual 4`;同一份 Go 代码 07-22×2/07-25×1 三次 CI 全绿,本地 -race ×200/×300 也全绿。
- **根因**：生产计算是 `time.Now().Unix() - relayInfo.StartTime.Unix()`(text_quota.go)——两端**各自截断到整秒**;测试夹具 `StartTime: time.Now().Add(-3*time.Second)`。夹具创建到扣费之间(建用户/渠道/token 等 DB 操作,d≈几十至几百 ms)只要跨过一次墙钟整秒边界,3 即变 4——**与 d 大小无关,纯看夹具时刻的小数秒落点**(确定性算术复现:小数 .850+d200ms→4)。翻车概率≈d 秒数,Mac d≈几 ms 所以几百连发都躲过,CI -race 慢跑 d 上百 ms → 15%+/次,两连红只是运气回归均值。绝对值断言测的不是契约(「重放逐字保真快照」),而是「测试自己跑得够不够快」。
- **解决/规避**：把断言改锚契约本身——读出实际快照 `snapshotUseTime := outbox.LogUseTime`,带宽 sanity(≥3 且 <60,证明确实源自 3s 前的 StartTime),重放侧改 `assert.Equal(snapshotUseTime, log.UseTime)`(逐字保真=测试真正要守的性质,反而比钉死 3 更严:此前 outbox 与 log 各自独立断 3,双双漂成 4 也能过)。gofmt/vet/单测×300/-race 全包全绿。全仓已 grep,无其他对 Unix 差值的绝对值断言。
- **升级**：候选规则——「测试断言**墙钟推导值**(Unix 差/Since 取整/日期字符串)的绝对量,一律视为时间刀锋:要么注入时钟,要么改断『派生关系+带宽』;code review 见 `assert.Equal(t, <常数>, xxx时长/秒差)` 即拦」。与「SetNX 守卫测试须可证伪」同族:那条治测试测不到竞态,本条治测试自身携带竞态。
