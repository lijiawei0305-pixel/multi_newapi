...
- **移动端（<1024px）从灯泡带发起的滑动不会触发页面滚动**：竖排布局下页面纵向滚动由 `.hero`
  自身的 `overflow-y:auto` 实现（见 `src/style.css` 竖排媒体查询注释），但中间灯泡区域
  （`.hero-stage`）始终保持 `.hero` 基础规则的 `pointer-events:none`——只有 `.hero-left`/
  `.hero-right`（文案栏、HUD 卡片）重新打开 `pointer-events:auto`。因此触摸滑动手势必须从
  文案栏或 HUD 卡片区域发起才能滚到底部；从中间灯泡带发起的滑动会穿透到底下的 WebGL 画布，
  不会被当作页面滚动处理。
- **这是有意为之的取舍，不是 bug**：`.hero-stage` 之所以让 `pointer-events` 穿透到画布，是为了
  让移动端用户能直接点按灯泡周围的卫星、触发 HUD 切换——「点击/点按卫星切换 HUD」是验收要求
  之一。如果反过来让灯泡带接管滚动手势，会连带吞掉 tap 事件，破坏这条 DoD。
- 已知影响：部分矮屏机型（如 390×844）上，用户若只在灯泡带滑动，页面不会滚动到底部 HUD 卡片，
  需改从文案区或已可见的 HUD 卡片区域滚动。已作为可接受的已知限制记录，本次不改代码。
