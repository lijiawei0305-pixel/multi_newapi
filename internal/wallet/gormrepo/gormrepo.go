...
func (r *Repo) RedeemCodeAndCredit(ctx context.Context, tenantID int64, code string, userID int64, now time.Time, quotaPerUnit float64) (amountUSD float64, creditUnits int64, err error) {
	err = r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if credit > 0 {
			...
			// Table("users") 不套软删作用域（入账到软删用户行也 OK：兑换者本已登录鉴权）；但**必须**校验
			// RowsAffected：若 userID 行根本不存在（0 行），静默成功会让已翻 used 的码作废却不到账——正是
			// 同文件 CreateCodesWithDeduction:209 已防、而此钱路曾漏的坑。0 行即 fail-loud 回滚，两路语义一致。
			...
			if res.RowsAffected == 0 {
				return wallet.ErrRedeemCreditUserMissing // 用户行不存在 → 回滚，CAS 撤销、码保持 enabled 可再兑
			}
		}
