...
//
// 粒度权衡：同一 NAT/校园网出口的不同真人共享出口 IP → 撞进同一指纹 → 连带收紧（第二人 Trial
// 被拒）。**缓解同 IP 连坐的正确手段**是设备维度有界 TTL（已在引擎层实现：device 去重键默认 24h
// 过期、长期误伤到点自动自愈，audit R1）或把限购占用挪到支付成功之后（零付款不占键）；个案可经
// 带鉴权+审计的后台释放通道（ReleaseTrialLimit）即时解套。**绝不能靠放粗网段**：先前此处注释称
// 「可把粒度退到 /24（IPv4）或 /64（IPv6）网段以降低连带误伤」，方向恰好反了——对「首占者赢 + 去重
// 键有生命周期」的模型，网段越粗 → 越多真人共用同一把键 → 连坐反而更重，与降低误伤背道而驰。
