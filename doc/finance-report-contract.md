...
### Locale files
Add English-keyed strings (keys are full English sentences; `nsSeparator:false`, so colons/`¥` in keys are fine) to `web/default/src/i18n/locales/zh.json` (the richest/canonical base) under the top-level `translation` map, then run `npm run i18n:sync` to propagate structure to `en/fr/ja/ru/vi.json`. `en.json` need not contain keys (i18next fallback-to-key).

### License header + route-tree regen
- Every new `.ts`/`.tsx` file MUST carry the exact 18-line QuantumNous AGPL header; verify with `npm run copyright:check` (add with `npm run copyright`).
- `routeTree.gen.ts` is auto-generated by the rsbuild `tanstackRouter` plugin (`rsbuild.config.ts`) on `npm run dev`/`build` — NEVER hand-edit; just run dev/build to register the two new routes.
...
| **S7 — i18n** | MODIFY `i18n/locales/*.json` (runs `npm run i18n:sync`) | S5/S6 string keys collected | **Last; single writer** (i18n:sync rewrites all locales) |
