...
## A. 接口与编排
- [ ] `ModelPermission.Check`（Token `model_allowlist` 校验）｜ ✅ 单测：不允许模型 `MODEL_NOT_ALLOWED`
- [ ] `UpstreamPool.Forward` 包装接口（复用 new-api 转发）｜ ✅ 单测（mock）：返回 resp+usage
- [ ] `RelayHandler.ChatCompletions` 编排：鉴权→租户活跃→风控→模型→选桶→转发→扣费→日志 ｜ ✅ **全 mock 依赖**单测：编排顺序与短路正确

## B. 关键路径
- [ ] 桶路由集成 `QuotaRouter`（有 active 套餐→套餐桶；否则钱包）｜ ✅ 单测：两路径选桶正确
- [ ] 扣费失败处理（套餐超额/过期/余额不足 → 拦截，不回退）｜ ✅ 单测：错误正确返回客户端
...
- [ ] 短路：鉴权失败不进风控、风控失败不转发 ｜ ✅ 单测断言调用未发生
- [ ] 冻结租户/超额套餐的调用被拦截 ｜ ✅ 测试红→绿
