...
## A. 接口与聚合
- [ ] `port.go`：`StatsService` ｜ ✅ `go build` 通过
- [ ] `AdminOverview`（全站租户数/活跃数/调用量/扣费/代理收益）｜ ✅ 单测：聚合数值正确
- [ ] `TenantOverview`（本站用户数/Token 数/调用量/收入/收益）｜ ✅ 单测：仅本租户范围
- [ ] `SubscriptionAlerts`（`used_usd/month_limit` ≥ 阈值如 80% 预警）｜ ✅ 单测：阈值边界正确
