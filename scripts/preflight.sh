...
set -euo pipefail
cd "$(dirname "$0")/.."
echo "==[preflight] 1/7 deploy 脚本 helper lint =="
# bash -n 抓不到"调用未定义函数"、本机无 shellcheck（audit#8：deploy.sh 调服务器侧才有的 warn → 运行期 127）。
bash scripts/lint-deploy-helpers.sh deploy/ops
echo "==[preflight] 2/7 gofmt（仅告警）=="
...
echo "==[preflight] 3/7 go build =="; go build ./...
echo "==[preflight] 4/7 go vet(internal) =="; go vet ./internal/...
echo "==[preflight] 5/7 go test internal(-race) =="; go test ./internal/... -race -count=1 >/dev/null
# 前端用 bun（web/bun.lock 为真源，勿用 pnpm）：workspace 根装依赖 → web/default 测试+构建。
echo "==[preflight] 6/7 frontend test =="
( cd web && bun install --frozen-lockfile >/dev/null )
( cd web/default && bun test )
echo "==[preflight] 7/7 frontend build =="
