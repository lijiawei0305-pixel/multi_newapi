...
func RequestWaffoPay(c *gin.Context) {
	...
	if !resp.IsSuccess() {
		responseMetadata := logger.PayloadMetadata([]byte(common.GetJsonString(resp)))
		logger.LogWarn(c.Request.Context(), fmt.Sprintf("Waffo 创建订单业务失败 user_id=%d trade_no=%s code=%s response_%s", id, merchantOrderId, resp.Code, responseMetadata))
		topUp.Status = common.TopUpStatusFailed
		_ = topUp.Update()
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": "拉起支付失败"})
		return
...
func WaffoWebhook(c *gin.Context) {
	if !isWaffoWebhookEnabled() {
		logger.LogWarn(c.Request.Context(), "Waffo webhook 被拒绝 reason=webhook_disabled")
	...
	if err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Waffo webhook 读取请求体失败 error_type=%T", err))
	...
	sdk, err := getWaffoSDK()
	if err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Waffo webhook SDK 初始化失败 error_type=%T", err))
	...
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Waffo webhook 收到请求 verification=pending signature_present=%t %s", signature != "", payloadMetadata))

	// 验证请求签名
	if !wh.VerifySignature(bodyStr, signature) {
		logger.LogWarn(c.Request.Context(), fmt.Sprintf("Waffo webhook 验签失败 verification=false %s", payloadMetadata))
		c.AbortWithStatus(http.StatusBadRequest)
		return
	}
	...
	if err := common.Unmarshal(bodyBytes, &event); err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Waffo webhook 解析失败 verification=true error_type=%T %s", err, payloadMetadata))
	...
	case core.EventPayment:
		...
		if err := common.Unmarshal(bodyBytes, &payload); err != nil {
			logger.LogError(c.Request.Context(), fmt.Sprintf("Waffo 支付回调载荷解析失败 event_type=%s error_type=%T %s", event.EventType, err, payloadMetadata))
			sendWaffoWebhookResponse(c, wh, false, "invalid payment payload")
			return
		}
		logger.LogInfo(c.Request.Context(), fmt.Sprintf("Waffo webhook 验签并解析成功 verification=true event_type=%s merchant_order_id=%s order_status=%s", event.EventType, payload.Result.MerchantOrderID, payload.Result.OrderStatus))
		handleWaffoPayment(c, wh, &payload.Result.PaymentNotificationResult)
	default:
		logger.LogInfo(c.Request.Context(), fmt.Sprintf("Waffo webhook 忽略事件 event_type=%s", event.EventType))
...
func handleWaffoPayment(c *gin.Context, wh *core.WebhookHandler, result *core.PaymentNotificationResult) {
	if result.OrderStatus != "PAY_SUCCESS" {
		logger.LogInfo(c.Request.Context(), fmt.Sprintf("Waffo 订单状态非成功，忽略充值 trade_no=%s order_status=%s", result.MerchantOrderID, result.OrderStatus))
	...
	if err := model.RechargeWaffo(merchantOrderId, c.ClientIP()); err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Waffo 充值处理失败 trade_no=%s error_type=%T", merchantOrderId, err))
		sendWaffoWebhookResponse(c, wh, false, err.Error())
		return
	}

	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Waffo 充值成功 trade_no=%s", merchantOrderId))
	sendWaffoWebhookResponse(c, wh, true, "")
}
