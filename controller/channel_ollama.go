...
func OllamaPullModel(c *gin.Context) {
	...
	err = ollama.PullOllamaModelWithContext(c.Request.Context(), baseURL, key, req.ModelName)
	if err != nil {
...
func OllamaPullModelStream(c *gin.Context) {
	...
	// 执行拉取
	err = ollama.PullOllamaModelStreamWithContext(c.Request.Context(), baseURL, key, req.ModelName, progressCallback)
...
func OllamaDeleteModel(c *gin.Context) {
	...
	key := strings.Split(channel.Key, "\n")[0]
	err = ollama.DeleteOllamaModelWithContext(c.Request.Context(), baseURL, key, req.ModelName)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"success": false,
...
func OllamaVersion(c *gin.Context) {
	...
	key := strings.Split(channel.Key, "\n")[0]
	version, err := ollama.FetchOllamaVersionWithContext(c.Request.Context(), baseURL, key)
	if err != nil {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
