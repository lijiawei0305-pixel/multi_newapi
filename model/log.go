...
type Log struct {
	...
	ProjectionKey     *string `json:"-" gorm:"type:varchar(64);uniqueIndex:idx_logs_projection_key"`
...
func RecordErrorLog(c *gin.Context, userId int, channelId int, modelName string, tokenName string, content string, tokenId int, useTimeSeconds int,
	...
	logger.LogInfo(c, fmt.Sprintf("record error log: userId=%d channelId=%d modelName=%s tokenId=%d content_%s", userId, channelId, modelName, tokenId, logger.PayloadMetadata([]byte(content))))
...
func RecordConsumeLog(c *gin.Context, userId int, params RecordConsumeLogParams) {
	...
	logger.LogInfo(c, fmt.Sprintf("record consume log: userId=%d channelId=%d modelName=%s tokenId=%d promptTokens=%d completionTokens=%d content_%s",
		userId, params.ChannelId, params.ModelName, params.TokenId, params.PromptTokens, params.CompletionTokens, logger.PayloadMetadata([]byte(params.Content))))
	username := c.GetString("username")
	requestId := c.GetString(common.RequestIdKey)
	upstreamRequestId := c.GetString(common.UpstreamRequestIdKey)
	createdAt := common.GetTimestamp()
