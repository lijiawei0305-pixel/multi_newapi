...
- 🟡 **[豁免待回收 · 到期 2026-08-31] 依赖审计豁免 GHSA-mh99-v99m-4gvg（brace-expansion,CVE-2026-14257）** —— 2026-07-25 CI 事故修复的遗留：该公告唯一补丁 5.0.8 改了 CJS 出口形状,web（classic 的 eslint 8 链）与 electron（electron-builder 26 钉死 asar 3.4.1 等）的旧 minimatch<10 栈无法安装,纯 dev/打包工具链暴露、不进产物,已进 `scripts/audit-allowlist.json` 显式豁免（到期自动重新阻断）。**回收条件**：brace-expansion 出回移植 / classic 迁 eslint 10 / electron-builder 26 线换 asar≥4——任一达成即删条目让真升级过门。每日 `dependency-audit` workflow 会在上游修复后提示「豁免未命中,请删除」。详见 RETRO.md 二·「代码零变化、CI 三 job 一夜变红」。
