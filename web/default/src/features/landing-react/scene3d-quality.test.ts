...
test('mid: phone-like high dpr or few cores', () => {
  ...
  const byCores = pickRenderQuality({
    dpr: 2,
    cores: 4,
    saveData: false,
    deviceMemory: 4,
...
test('low: saveData / low memory / dual core', () => {
  expect(pickRenderQuality({ dpr: 3, cores: 8, saveData: true }).tier).toBe(
    'low'
  )
  expect(pickRenderQuality({ dpr: 2, cores: 8, deviceMemory: 2 }).tier).toBe(
    'low'
  )
