...
function EarningsCalculator() {
  ...
  const pkgFields = [
    {
      id: 'pkgPrice',
      label: t('Agent Join Calc Pkg Price', { defaultValue: '套餐售价（元）' }),
      value: pkgPrice,
      onChange: setPkgPrice,
    },
    {
      id: 'pkgCost',
      label: t('Agent Join Calc Pkg Cost', {
        defaultValue: '你的底价 / 进货成本（元）',
      }),
      value: pkgCost,
      onChange: setPkgCost,
    },
    {
      id: 'pkgCount',
      label: t('Agent Join Calc Pkg Count', { defaultValue: '月售套餐数' }),
      value: pkgCount,
      onChange: setPkgCount,
    },
  ]
  const apiFields = [
    {
      id: 'baseCost',
      label: t('Agent Join Calc Api Base', {
        defaultValue: '月基准消耗额（元 · 按上游成本计）',
      }),
      value: baseCost,
    ...
    {
      id: 'userRate',
      label: t('Agent Join Calc Api User Rate', {
        defaultValue: '你给用户的倍率',
      }),
      value: userRate,
    ...
    {
      id: 'bottomRate',
      label: t('Agent Join Calc Api Bottom Rate', {
        defaultValue: '平台给你的底价倍率',
      }),
      value: bottomRate,
...
export function AgentJoin() {
  ...
  const steps = [
    ...
    t('Agent Join Step 3', {
      defaultValue:
        '在域名服务商添加 CNAME 记录，指向指定地址（联系客服获取）。',
    }),
    t('Agent Join Step 4', {
  ...
  const fallbackPlans: PlanCard[] = [
    {
      ...
      anchor: '1,980',
      price: '990',
      period: t('Agent Join Plan Validity Year', {
        defaultValue: '有效期 365 天',
      }),
    ...
    livePlans && livePlans.length > 0
      ? [...livePlans]
          ...
          .map((p) => ({
            ...
            currency: '¥',
            anchor:
              p.anchor_price_cny > p.price_cny
